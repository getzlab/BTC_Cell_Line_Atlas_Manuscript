...
## CHOL cell lines vs other GI cancers
GI cancers: pancreatic, gastric, colon, liver, gastric.
Since our CHOL TPM matrix is 20Q2, the TPM matrix of the other GI cancer was obtained from [DepMap Public 20Q2 Primary Files](https://depmap.org/portal/download/all/). \break

```{r intermediate gi}
rna_file = 'Main_Data/RNA/rna_cell_lines.csv'
# data_transcript = read.csv(rna_file, row.names = 'Gene')
data_transcript <- load_csv_file(paste0(input_directory, rna_cl_file), row_name = 'Gene')
## CHOL cell line key
icc_key <- key %>% filter(RNA == "1" & Subtype != 'HCC') #56

## Read RNA-seq matrix - CHOL cell lines
data_transcript_cl <- data_transcript[, which(colnames(data_transcript) %in% icc_key$Cell_line)]
# row.names(data_transcript_pdx_cl_common) = data_transcript$Gene
data_transcript_cl <- data_transcript_cl[, order(match(names(data_transcript_cl), icc_key$Cell_line))]


## other GI cancer key
depmap_sample_info_file = 'Additional_Data/DepMap/RNA_20Q2/public-20q2_v18-sample-info.csv'
other_annot <- read.csv(paste0(input_directory, depmap_sample_info_file))
other_annot <- other_annot %>% filter(!(DepMap_ID %in% icc_key$DepMap_ID))
other_annot <- other_annot %>% filter(lineage == "pancreas" | lineage == "colorectal" | lineage =="gastric" |
                                        lineage == "liver" | lineage == "esophagus")#256
## Read RNA-seq matrix - other GI cancer lines
depmap_rna_file = 'Additional_Data/DepMap/RNA_20Q2/public-20q2_v18-ccle-expression.csv'
data_transcript_all <- read.csv(paste0(input_directory, depmap_rna_file), row.names = 1)
data_transcript_all <- data_transcript_all[!(rownames(data_transcript_all) %in% icc_key$DepMap_ID), ]
data_transcript_all <- data_transcript_all %>% filter(rownames(data_transcript_all) %in% other_annot$DepMap_ID)
name <- as.vector(colnames(data_transcript_all))
new.name <- str_extract(colnames(data_transcript_all), "[^..]+")
colnames(data_transcript_all) = new.name
```

### UMAP calculation
```{r gi-1, fig.width=8, fig.height=8}
set.seed(seed)

## numbers of genes are different in these two RNA-seq matrix! I took genes that shared between the two matrix.
# N = 17564 genes shared across two different gene matrix.
gene.key <- intersect(colnames(data_transcript_all), row.names(data_transcript_cl) )

## combine RNA matrix
# data_transcript_all.2 <- column_to_rownames(data_transcript_all, "X")#
data_transcript_all.3 <- data_transcript_all[, gene.key] %>% base::t()
dim(data_transcript_all.3)

data_transcript_cl.2 <- data_transcript_cl[gene.key, ]
dim(data_transcript_cl.2)

data_transcript_cl.merged <- cbind(data_transcript_cl.2, data_transcript_all.3)
dim(data_transcript_cl.merged)

## keep only the genes that have > log2(10) summed across the transcripts
# 17564 transcripts to 17161 transcripts
cutoff_expr <- log2(10)
data_transcript_pdx_cl_common.10 <- data_transcript_cl.merged %>%
  mutate(sum = rowSums(.)) %>%
  filter(sum > cutoff_expr) %>%
  select(-sum)

## Make colData file for a DESeq2 subject
colData.1 <- data.frame(sample = icc_key$Cell_line,
                      Lineage = c(rep("bile duct", 56)))

colData.2 <- data.frame(sample = other_annot$DepMap_ID,
                        Lineage = other_annot$lineage) %>%
  filter(sample %in% colnames(data_transcript_all.3))

colData.3 <- rbind(colData.1, colData.2)

## check sample # per GI cancer type
table(colData.3$Lineage)

## Version A. Clustering using threshold > log2(10)
# A-1. convert log2 table back to TPM table
data_transcript_pdx_cl_common_TPM <- data_transcript_pdx_cl_common.10^2

# A-2. Verify that sample names and colnames are the same
# make sure sample names are in the same order in sample and count table files
all(colData.3$sample %in% colnames(data_transcript_pdx_cl_common_TPM))
all(colData.3$sample == colnames(data_transcript_pdx_cl_common_TPM))
cts <- as.data.frame.matrix( data_transcript_pdx_cl_common_TPM[, colData.3$sample] )
...
names(umap_plot_df) = c("sample", "UMAP1", "UMAP2", "Lineage")


write_csv(umap_plot_df, paste0(output_directory, 'UMAP_flat_GI_cell_lines.csv'))
...
### UMAP visual
```{r gi-2, warning = F, fig.width=8, fig.height=8}

## Format color

color <- hcl.colors(8, palette = "Set2")

cust.color = c("bile duct" = "#fb6a4a", "esophagus" = color[8],
                              "gastric" = color[4], "liver" = color[2], "colorectal" = "grey30", "pancreas" = color[6])

## read UMAP flat file
umap_plot_df <- read.csv(paste0(output_directory, "UMAP_flat_GI_cell_lines.csv"), row.names = 1)

## plotting
ggplot(umap_plot_df, aes(x = UMAP1, y = UMAP2, color = Lineage)) +
  ggtitle("DepMap - GI cancers") +
  geom_point(size = 5,shape = 16, aes( color = Lineage), alpha = 0.7) +
  scale_color_manual(values = cust.color) +
  theme_bw() +
  theme(text = element_text(size = 10),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
ggsave(paste0(output_directory, 'RNAseq_UMAP_GI_cell_lines.pdf'), units = 'in', width = 5, height = 4)
```
...
Specifically, we use CHOL cholangiocarcinoma cohort (n = 51), COADREAD Colorectal adenocarcinoma cohort (n = 631), PAAD Pancreatic adenocarcinoma (n = 185), STAD Stomach adenocarcinoma (n = 443), ESCA Esophageal carcinoma (n = 185). Download from `mRNA-seq Preprocess package` and use `_RSEM_all` file. This analysis was done using `RSEM` read file. \break
...
depmap_sample_info_file = 'Additional_Data/DepMap/RNA_20Q2/public-20q2_v18-sample-info.csv'
other.key <- read.csv(paste0(input_directory, depmap_sample_info_file))
...
depmap_rna_reads_file = 'Additional_Data/DepMap/RNA_20Q2/public-20q2_v18-ccle-rnaseq-reads_cleaned.csv'
...
# name <- as.vector(colnames(data_transcript_GI))
# new.name <- str_extract(colnames(data_transcript_GI), "[^..]+")
# colnames(data_transcript_GI) = new.name

# write.csv(data_transcript_GI, paste0(work_dir, 'RSEM_RNAseq_reads_20Q2_GI.csv'), row.names = T)

data_transcript_GI_t <- t(data_transcript_GI) #58567
...
gene.key.final <- intersect(rownames(data_tcga), rownames(data_transcript_cl_filtered)) #15709
...
ggplot(umap_plot_df, aes(x = UMAP1, y = UMAP2, shape = source)) +
  ggtitle('DepMap and TCGA – GI cancers')+
    geom_point(data = umap_plot_df_pat, size = 4, aes(shape = source, color = lineage), alpha = 1/3) +
    geom_point(data = umap_plot_df_cl, size = 4, aes(shape = source, color = lineage), alpha = 0.4) +
  geom_point(data = umap_plot_df_cl_chol, size = 4,  aes(shape = source, color = lineage), alpha = 0.5) +
  geom_point(data = umap_plot_df_chol, size = 5.5,  aes(shape = source, color = lineage), alpha = 1) +
  ...
  scale_shape_manual(values = c("cell line" = 16, "patient tumor" = 3)) +
  theme_bw() +
...
# geom_point(data = umap_plot_df_chol, size = 8, aes(shape = source, color = lineage)) +
      geom_point(data = umap_plot_df_pat, size = 8, aes(shape = source, color = lineage), alpha = 1) +
    geom_point(data = umap_plot_df_cl, size = 8, aes(shape = source, color = lineage), alpha = .7) +
  geom_point(data = umap_plot_df_cl_chol, size = 8,  aes(shape = source, color = lineage), alpha = .7) +
  geom_point(data = umap_plot_df_chol, size = 9.5,  aes(shape = source, color = lineage), alpha =1) +
   scale_colour_manual(values = c("bile duct" = "#fb6a4a", "esophagus" = color[8],
                              "gastric" = color[4], "liver" = color[2], "colon" = "grey30", "pancreas" = color[6])) +
  scale_shape_manual(values = c("cell line" = 16, "patient tumor" = 3)) +
